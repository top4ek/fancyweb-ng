...
export default function Input({ elemName, label, borderWidth, borderColor }: InputProps) {
  ...
  return (
    <div className={`border ${borderWidthClasses[borderWidth]} ${borderColorClasses[borderColor]} rounded-md flex flex-col has-[:focus]:outline has-[:focus]:outline-[4px] has-[:focus]:outline-stages-border bg-wallet-bg`}>
      ...
      <input name={elemName} id={elemName} className="pl-1 py-1 m-2 font-mono text-xl h-7 focus:border-none focus:outline-none"/>
    </div>
  );
}
