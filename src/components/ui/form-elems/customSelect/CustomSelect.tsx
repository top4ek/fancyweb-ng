...
export default function CustomSelect({ state, value, onChange, options, open, size, elemName }: SelectProps) {
  ...
  function selectStyleFab(state: SelectProps['state']) {
    ...
    const baseStyle = `${sizer[size ?? 'md']} w-full px-2 outline outline-0 border rounded hover:outline-1 focuse:outline-none flex flex-row items-center cursor-default relative`;
